...
public:
  IfStmt(sona::owner<Expr> thenExpr, SourceLocation ifLocation,
         SourceLocation leftParenLocation, SourceLocation rightParenLocation)
      : Stmt(StmtId::SI_If), m_ThenExpr(std::move(thenExpr)),
        m_ElseExpr(sona::empty_optional()), m_IfLocation(ifLocation),
  ...
  IfStmt(sona::owner<Expr> thenExpr, sona::owner<Expr> elseExpr,
         SourceLocation ifLocation, SourceLocation leftParenLocation,
         SourceLocation rightParenLocation, SourceLocation elseLocation)
      : Stmt(StmtId::SI_If), m_ThenExpr(std::move(thenExpr)),
        m_ElseExpr(std::move(elseExpr)), m_IfLocation(ifLocation),
        m_LeftParenLocation(leftParenLocation),
        m_RightParenLocation(rightParenLocation), m_ElseLocation(elseLocation) {
  }
  ...
  SourceLocation GetIfLocation() const noexcept { return m_IfLocation; }
...
public:
  ...
  ForStmt(T1 initExpr, T2 condExpr, T3 incrExpr, sona::owner<Stmt> &&stmt,
          SourceLocation forLocation, SourceLocation leftParenLocation,
      ...
      : m_InitExpr(std::move(initExpr)), m_CondExpr(std::move(condExpr)),
        ...
        m_ForLocation(forLocation), m_LeftParenLocation(leftParenLocation),
        m_RightParenLocation(rightParenLocation),
  ...
  SourceLocation GetForLocation() const noexcept { return m_ForLocation; }
...
private:
  ...
  sona::owner<Stmt> m_Stmt;
  SourceLocation m_DoLocation, m_WhileLocation, m_LeftParenLocation,
