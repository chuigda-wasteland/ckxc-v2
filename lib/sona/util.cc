...
void assert_impl(bool cond, const char *file, int line,
                 const char *func, const char *cond_str, const char *desc) {
    if (!cond) {
        std::fprintf(
            stderr, "assertion %s failed at file %s, line %d, func %s : %s\n",
